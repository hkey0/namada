...
mod test_utils {
    ...
    impl TestShell {
        ...
        pub fn new_at_height<H: Into<BlockHeight>>(
            ...
            shell.state.in_mem().block.height = height.into();
        ...
        pub fn enqueue_tx(&mut self, tx: Tx, inner_tx_gas: Gas) {
            self.shell.state.in_mem().tx_queue.push(TxInQueue {
                tx,
                gas: inner_tx_gas,
            });
        }
        ...
        pub fn start_new_epoch_in(&mut self, num_blocks: u64) {
            ...
            self.state.in_mem().next_epoch_min_start_time = DateTimeUtc::now();
        }
        ...
        pub fn start_new_epoch(&mut self, req: Option<FinalizeBlock>) -> Epoch {
            ...
            if let Some(LastBlock { height, .. }) =
                self.state.in_mem().last_block.as_mut()
            {
    ...
    fn test_tx_queue_persistence() {
        ...
        shell.state.in_mem().tx_queue.push(TxInQueue {
            tx: wrapper,
            gas: u64::MAX.into(),
        });
