...
mod tests {
    ...
    fn test_vp_memory_limiter_in_guest_calling_eval() {
        let mut state = TestState::default();
        let addr = state.in_mem().address_gen.generate_address("rng seed");
    ...
    fn test_vp_memory_limiter_in_host_input() {
        let mut state = TestState::default();
        let addr = state.in_mem().address_gen.generate_address("rng seed");
        let mut gas_meter = RefCell::new(VpGasMeter::new_from_tx_meter(
            &TxGasMeter::new_from_sub_limit(TX_GAS_LIMIT.into()),
        ));
        let keys_changed = BTreeSet::new();
        let verifiers = BTreeSet::new();
    ...
    fn test_tx_memory_limiter_in_host_env() {
        ...
        // Borsh.
        state.write(&key, value.serialize_to_vec()).unwrap();
